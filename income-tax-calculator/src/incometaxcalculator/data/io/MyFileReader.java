...
public abstract class MyFileReader {
  ...
  protected abstract boolean checkForReceipt(String[] values);
  protected abstract int getValueNumberForParsing();
  protected abstract String processValues(String[] values);
  ...
  protected int checkForReceipt(BufferedReader inputStream) throws NumberFormatException, IOException {
    ...
    while (!isEmpty(line = inputStream.readLine())) {
      ...
      if(checkForReceipt(values)) {
        int receiptId = Integer.parseInt(values[getValueNumberForParsing()].trim());
        return receiptId;
      }
    }
    return -1;
